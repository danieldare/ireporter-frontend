  ...
  fetch(request)
    .then(response => {
      if(response.status === 200){
        window.location.href = 'view-redflags.html'
      }
      return response.json();
    })
    .then(data => {
  ...
  .then(response => {
    ...
    .then(data => {
      ...
      if(data.errors){
        ...
        document.getElementById('delete-msg').innerHTML = `<h4 style="text-align:center; color: green">Updated red-flag record’s comment.</h4>`;
